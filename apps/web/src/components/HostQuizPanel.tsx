...
import { startQuizThunk, submitQuizAnswerThunk, revealQuizThunk } from '../store/slices/quizSlice';
...
export function HostQuizPanel() {
  ...
  const loading = useAppSelector((s) => s.quiz.loading);
  ...
  const handleReveal = () => {
    ...
    const awards = session.teams
      .filter((team) => team.participants.some((p) => quizState.answers.some((ans) => ans.participantId === p.id && ans.answer === question.correct)))
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={handleStart}
            disabled={loading || (quizState?.status === 'running')}
          >
